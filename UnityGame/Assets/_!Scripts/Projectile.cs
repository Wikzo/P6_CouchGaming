...
public class Projectile : MonoBehaviour
{
    ...
    // added by Gustav
    void DestroyProjectileAndClone(PlayerAim playerAim)
    {
    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Raycast(transform.position, -transform.right, out hit, transform.localScale.x / 2) || Physics.Raycast(transform.position, transform.right, out hit, transform.localScale.x / 2))
        {
            ...
            {
                // put into function by Gustav
                // hit.collider.gameObject.GetComponent<PlayerAim>().CurrentShotAmount++;
                //Destroy(gameObject);
                DestroyProjectileAndClone(hit.collider.gameObject.GetComponent<PlayerAim>());
            }
            // added extra check for name, by Gustav
            if (!hit.collider.gameObject.GetComponent<PlayerDamage>() && hit.collider.gameObject.tag != "NotCollidable" && hit.collider.gameObject.name != gameObject.name)
            {
